...
class ScalarProduct(Scene):
    ...
    def angle_of_vectors_algebric(self):
        ...
        where_text = (
            TextMobject(
                "Where, |$\\vec{A}$|, |$\\vec{B}$| are magnitude of $\\vec{A}$ and $\\vec{B}$ resp."
            )
            .next_to(cosine_formula, DOWN, buff=0.2)
            .to_edge(DOWN)
        )
    ...
    def dot_product_properties(self):
        ...
        commutative_g = (
            VGroup(*[TextMobject(i) for i in texts])
            .arrange(DOWN, center=False)
            .next_to(commutative, DOWN, buff=1.0)
        )
        texts = (
        ...
        associative_g = (
            VGroup(*[TextMobject(i) for i in texts])
            .arrange(DOWN, center=False)
            .next_to(associative, DOWN, buff=1.0)
        )
        texts = (
        ...
        distributive_g = (
            VGroup(*[TextMobject(i) for i in texts])
            .arrange(DOWN, center=False)
            .next_to(distributive, DOWN, buff=1.0)
        )
