...
class Example(Scene):
    ...
    def showq(self):
        ...
        question = (
            TexMobject(
                "\\text{The birth rate of the population of a town is } 8 \\% \\text{ per year and death rate is } 2 \\% \\text{ per year.}",
                "\\text{If the population of the town is } 6,00,000 \\text{. Find the population before 3 years.}",
            )
            .scale(0.7)
            .shift([0, 2, 0])
    ...
    def solMath(self):
        fInfo = (
            VGroup(
                ...
                TexMobject(
                    "\\text{Rate of increase of Population = } 8\\% - 2\\% = ", "6\\%"
                ).arrange(RIGHT),
                TexMobject("\\text{Present Population } (P_t) = ", "6,00,000").arrange(
                    RIGHT
                ),
            )
            .arrange(DOWN, aligned_edge=LEFT)
        ...
        f5 = TexMobject("\\text{Therefore, }", "P =", "503771.569").shift(
            [-1.3, -1.5, 0]
        )
        ...
        t1 = (
            TextMobject(
                "The population before 3 years was 503772. ",
                "(As population is discrete. We usually round it off)",
            )
            .shift([-1, -2.5, 0])
            .scale(0.7)
        )
