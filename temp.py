...
class Theorems(Scene):
    ...
    def theorem1_proof(self):
        ...
        self.theorem_fig.next_to(topic, buff=1.0).shift(DOWN)
        para, para2, *labels2 = self.theorem_fig.submobjects
        A, D, C, B = para.get_vertices()
        F, E, C, B = para2.get_vertices()

        self.play(Write(topic))
        self.play(Write(self.theorem_fig))
        ...
        animations = [
            ([Animation(self.theorem_fig)], 0.01),
    ...
    def theorem2(self):
        ...
        BCDE_fig = parallelogram.copy().scale(0.2)
    ...
    def theorem2_proof(self):
        ...
        self.theorem_fig.next_to(topic, buff=1.0).shift(DOWN)
        para, triangle_abc, para2, *labels2 = self.theorem_fig.submobjects
    ...
    def theorem3(self):
        ...
        statement_t = TextMobject("Statement:")
        statement = TextMobject(
            "Two triangles standing on the same base and lying between"
        )
        statement_ = TextMobject(
        ...
        big_quad.shift(DOWN)
        A, E, D, C, B = big_quad.get_vertices()
        ...
        given3.next_to(given2, DOWN, buff=0.2).to_edge(LEFT)
        given4.next_to(given3, DOWN, buff=0.2).to_edge(LEFT)
        construction = TextMobject("Construction: CE || AB is drawn")
    ...
    def theorem3_proof(self):
        topic = TextMobject("Proof of Theorem 3", color=BLUE).scale(1.2)
        topic.to_edge(UL)

        self.theorem_fig.next_to(topic, buff=1.0).shift(DOWN)
        big_quad, *_ = self.theorem_fig.submobjects
        A, E, D, C, B = big_quad.get_vertices()
        ...
        col_2_data = (
            "Statements",
            r"$\therefore$ ABCE is a parallelogram",
            r"$\triangle$ ABC = $\frac{1}{2}$ $\square$ ABCE",
            r"$\triangle$ BCD = $\frac{1}{2}$ $\square$ ABCE",
            r"$\therefore$ $\triangle$ BCD = $\triangle$ ABC",
        )
        ...
        col_3_data = (
            ...
            "Opposite side being parallel",
            "Diagnol of a parallelogram bisects it",
            ("Triangle and parallelogram", "standing on same base"),
            "From (2) and (3)",
        )

        para_abce = Polygon(A, B, C, E, fill_color=YELLOW, fill_opacity=1.0)
        tri_abc = Polygon(A, B, C, fill_color=YELLOW, fill_opacity=1.0)
        tri_bcd = Polygon(B, C, D, fill_color=YELLOW, fill_opacity=1.0)

        animations = [
            ([Animation(self.theorem_fig)], 0.01),
            ([ShowCreationThenFadeOut(para_abce)], 3),
            (
                [
                    ShowCreationThenFadeOut(tri_abc),
                    ShowCreationThenFadeOut(para_abce),
                ],
                3,
            ),
            (
                [
                    ShowCreationThenFadeOut(tri_bcd),
                    ShowCreationThenFadeOut(para_abce),
                ],
                3,
            ),
        ...
        col_1 = self.makeColGroup(col_1_data)
        col_2 = self.makeColGroup(col_2_data, text_scale=0.85)
        col_3 = self.makeColGroup(col_3_data, text_scale=0.85)
        col_1.to_edge(LEFT)
        col_2.next_to(col_1)
        col_3.next_to(col_2)
        row_objs = [col_1.submobjects, col_2.submobjects, col_3.submobjects, animations]
    ...
    def makeColGroup(self, texts, tex_class=TextMobject, spacing=0.5, text_scale=1.0):
        ...
        for text in texts:
            ...
            if isinstance(text, str):
                ...
                objects = [tex_class(i).scale(0.85) for i in text]
                processed = [objects[0]]
                for i in objects[1:]:
